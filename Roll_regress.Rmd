...
df$TRM <- DGS.clean$TRM0503

df.expansionary <- subset(df, dum== 0)
df.recessionary <- subset(df, dum== 1)

colnames(df)
col_order <- c("Date", "GDP", "YIV", "dum", "log_gdp", "H3", "H6", "H9", "H12", "H15", "H18", "H21", "H24", "H27", "H30", "H33", "H36", "TRM")
df <- df[, col_order]
...
{ # var - the name of the variable to be regressed
  res <-roll_regres(as.formula(paste0(var, "~ TRM")), df[1:91,], width=20L, do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
  predicted <- unlist(res[4])
  predicted <- predicted[21:91]
  actual <- df[[var]][21:91]
  RMSFE <- rmse(preds = predicted, actuals = actual)
...
df_results <- rbind(df_results1, df_results2, df_results3, df_results4, df_results5)
...
rownames(df_results) <- c("Out-of-sample RMSFE", "Recessionary", "Expansionary" ,"Naive", "TRM")
