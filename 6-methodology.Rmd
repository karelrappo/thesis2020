...
First is the randomness part - random forest uses bootstrap aggregation (bagging) to construct random samples of the initial training dataset. Furthermore, the randomness is also included in variable selection - it randomly selects x number of variables of the total set X used for splitting at each node.
...
5) This process is repeated until each terminal node reaches minimum size (by default 5 observations).
