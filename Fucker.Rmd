...
do_regression <- function(var)
  { # var - the name of the variable to be regressed
    res <- dynlm(as.formula(paste0(var, "~ YIV+dum")), df) # run linear regression
    coefs <- c("R-Squared"= summary(res)$r.squared*100,
               "R-Squared (adj.)"=summary(res)$adj.r.squared*100, 
               "Intercept" = summary(res)$coefficients[1], 
               "Beta" = summary(res)$coefficients[,2], 
               "t-statistic" = summary(res)$coefficients[2,3], 
               "p-value"= summary(res)$coefficients[4],
               "newey"=NeweyWest(res)[,2])
