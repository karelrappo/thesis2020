...
## Random Forest 
In the research, we focus on a random forest thanks to its ability to account for complex nonlinearities. Random forest itself is a supervised learning method i.e. it predicts outputs based on existing input and output pairs. The concept of random forest in essence is relatively simple. It builds upon the decision trees i.e. random forest consists of multiple standalone decision trees.

\begin{figure}
\includegraphics{trees}
\caption{Example of a decision tree. Figure extracted from Niculaescu, O. (2018).}
\label{decisiontree}
\end{figure}


Figure \@ref(decisiontree) above depicts a single decision tree. The example here uses variables and makes a simple logical decision (yes/no) based on whether the variable’s value is bigger or smaller than the comparison variable. Through these kinds of logical iterations the decision tree arrives at the end value. Furthermore, the random forest then is an average of the individual decision trees.
