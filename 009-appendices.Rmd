...
end <- as.Date("01/12/2015", format="%d/%m/%Y")
...
fred_quarterly <- fredqd(date_start=start,date_end=end)
fred_quarterly <- subset(fred_quarterly, select = -c(X91, X181, X183))

variable_names <- as.vector(colnames(fred_quarterly[,-1]))

dataset_overview <- describe_qd(variable_names, name.only = FALSE)
```
