...
# siit kaotasin ära intercept selle filteriga
...
```{r child = 'archieve.rmd', include=FALSE}
...
```{r vsextracted, message=F, warnings=F,echo = F, fig.height=3, fig.cap="Actual vs Extracted GDP growth rate in %"}
...
```{r regfit, message = F, warnings = F, echo = F, fig.height=3, fig.width=6, fig.cap="Regressions' R-squared"}
  ...
  kbl(booktabs = T,longtable=T, align="c", digits=2, caption="Regression with state-dependency") %>%
   kable_styling(latex_options = c("striped"), full_width = TRUE) %>%
     column_spec(1:1, width="2in")%>%
       ...
       footnote(general = "*** - p<0.01, ** - p<0.05, * - p<0.1. Reported standard error is adjusted for heteroskedasticity")
#model3 <- lm(log_gdp ~ YIV + lag1 + lag2, df)
#ols_vif_tol(model3)
...
regr4 <- regrs(mudelid=c("lag1", "lag2", "DGS1","TRM1012", "SRT03M", "baa_aaa", "VIX", "housng", "gz_spr", "spy_logreturn"))
...
regr5 <- regrs(mudelid=c("YIV", "dum", "lag1", "lag2", "DGS1", "TRM1012", "SRT03M", "baa_aaa", "VIX", "housng", "gz_spr", "spy_logreturn"))
...
```{r MRF, message = F, warnings = F,echo = F}
#RMSE Subject to oos period and variable numbers
...
RMSE<-rmse(preds=predicted$value,actuals=predicted$GDP)
