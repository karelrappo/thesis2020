...
fred_quarterly <- read_csv("data/current.csv")


fred_quarterly <- subset(fred_quarterly, select = -c(92, 182, 184))
fred_quarterly <- fred_quarterly[-1:-2,]
fred_quarterly$sasdate <- as.Date(fred_quarterly$sasdate, format="%m/%d/%Y")
colnames(fred_quarterly)[1] <- "Date"

fred_quarterly <- subset(fred_quarterly, Date >= start)
fred_quarterly <- subset(fred_quarterly, Date <= end)

rownames(fred_quarterly) <- seq(length=nrow(fred_quarterly))
YIV <- yivgdp$YIV
