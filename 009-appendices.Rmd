...
regression.fit <- ggplot(results1, aes(x=row.names(results1),y=r.squared)) + geom_bar(stat="identity", fill="steelblue")+ theme_bw() + xlab("Quarters") + ylab("R-squared") + labs(title = "GDP R-squared") +  theme(plot.title = element_text(hjust = 0.5)) + scale_x_discrete(limits=c("H1","H2","H4", "H8"))
...
regression.coefficients <- ggplot(results1, aes(x=row.names(results1), y=YIV_estimate))+ geom_bar(stat="identity", fill="steelblue")+theme_bw()+ xlab("Quarters") + ylab("Coefficients") + labs(title = "GDP coefficients") + theme(plot.title = element_text(hjust = 0.5)) + scale_x_discrete(limits=c("H1","H2","H4","H8"))
...
regr5 <- regrs(mudelid=c("YIV", "dum", "DGS1", "TRM1012", "SRT03M", "baa_aaa", "VIX", "housng", "gz_spr", "spy_logreturn"),interaction=F)
...
\newpage
## Appendix F^[Full R-Code with data is available upon request in github repository: https://github.com/karelrappo/thesis2020]


```{r message = F, warnings = F,echo = F, fig.cap="RMSFE-s of linear model and random forest"}

ggplot(rf_resultsss, aes(variable, value, group=factor(Specification))) + geom_line(aes(color=factor(Specification))) + theme(legend.position="bottom") + labs(x="Time horizon", y="RMSFE", color="Model") + 
  facet_wrap(~period) + scale_color_manual(values=c("blue", "red")) + theme_bw()
