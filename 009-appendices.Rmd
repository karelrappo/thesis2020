...
In Panel A, YIV data is summarized. 
...
In Panel C, different control variables are listed: 
...
statistics %>%
  select(-N.Valid) %>%
  arrange(match(Variable, c("YIV", "GDP")), desc(Mean)) %>%
...
mrf.output=MRF(data=MRF.data,y.pos=1,x.pos=2:3,S.pos=2:20,oos.pos=75:103)

predicted <- as_tibble(mrf.output$pred)
predicted <- predicted %>%
  mutate(GDP = df[75:103,]$GDP)
