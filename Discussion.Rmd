...
Also, it was concluded that the biggest forecasting gains of advanced models tend to be around market turndowns. On the CSSFED plot, it can be seen that big jumps mostly exist occur around the recessionary periods. This means that random forest model has superior performance around recessionary periods. To analyze the matter in-depth, one can examine the models' predicted values compared to the actual GDP growth(appendix  \@ref(S:predictedactuals)). Comparing the two models' predictions to actuals, it can be noticed that random forest fails to fully estimate the declines in GDP during recessions - i.e. it underestimates compared to the linear model. Nevertheless, as the linear models have random spikes, especially during and after recessions, its error is higher. Hence, our results are aligned with the claims of @Siliverstovs2020 & @Siliverstovs2021 and @Chauvet2013 according to whom advanced methods that tackle non-linearities are expected to outperform simple models during turmoils. Also, the fact that random forest fails to reach the depths of recessions with its predictions is consistent with @Zhang2012 who states the underestimation during such recessionary period is a common problem with random forest. To elaborate, this happens due to RF algorithms as it uses averaging when making predictions - thus, the extreme values tend to be underestimated and smaller values overestimated. 
